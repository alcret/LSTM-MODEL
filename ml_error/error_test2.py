...
def lstm(batch):
    w_in = initial.weights['in']  # 选取输入的权重
    b_in = initial.biases['in']  # 选取输入的偏执量
    input = tf.reshape(initial.X, [-1, initial.input_size], name='input')  # 将每批次输入的X进行降维 X[None,20,1] ---> [-1,1]
    input_rnn = tf.matmul(input, w_in) + b_in  # 计算隐藏层    shape [-1,10]+[10,]
    input_rnn = tf.reshape(input_rnn, [-1, initial.time_step, initial.rnn_unit], name='input_rnn')  # 隐藏层变形 成三维张量(升维)
    cell = tf.nn.rnn_cell.BasicLSTMCell(initial.rnn_unit, 1)
    ...
    output = tf.reshape(output_rnn, [-1, initial.rnn_unit], name='output')  # 输出层变形为二维张量（降维）
    w_out = initial.weights['out']  # 最后一层权重
    b_out = initial.biases['out']  # 最后一层偏执量
...
def train_lstm():
    ...
    loss = tf.reduce_mean(tf.square(tf.reshape(pred, [-1]) - tf.reshape(initial(df).Y, [-1])), name='loss')  # 代价函数,梯度下降法，类似欧几里德距离算法
    train_op = tf.train.AdamOptimizer(initial.lr, name='train_op').minimize(
    ...
    with tf.Session() as sess:
        ...
        for i in range(1000):  # 迭代一百次训练
            step = 0
            start = 0
            end = start + initial.batch_size
            while (end < len(initial.train_x)):
                _, loss_ = sess.run([train_op, loss], feed_dict={initial.X: initial.train_x[start:end], initial.Y: initial.train_y[start:end]})
                start += initial.batch_size
                end = start + initial.batch_size
                if step % 2 == 0:
                    print("训练次数:", i, "脚步", step, "损失函数:", loss_)
                    print("保存", saver.save(sess, initial().module_file))
                step += 1
...
def prediction():
    ...
    with tf.Session() as sess:
        ...
        saver.restore(sess, initial.module_file)

        # 训练集最后一行测试
        prev_seq = initial.train_x[-1]
        predict = []
        predicts = []
        for i in range(1):
            next_seq = sess.run(pred, feed_dict={initial.X: [prev_seq]})
            predict.append(next_seq[-1])
            prev_seq = np.vstack((prev_seq[1:], next_seq[-1]))  # 每次得到最后一个时间步的预测结果，与之前的数据加在一起，形成新的测试样本

        plt.figure()
        plt.plot(list(range(len(initial.normalize_data))), initial.normalize_data, color='r')
        plt.plot(list(range(len(initial.normalize_data), len(initial.normalize_data) + len(predict))), predict, color='b')
        plt.show()

        for x in predict:
            predicts.append(x * np.std(initial.data) + np.mean(initial.data))
        print(predicts[0])
