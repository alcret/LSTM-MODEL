...
class PREDICTION:
    def prediction(names):
        with tf.variable_scope(str(names), reuse=True):
            pred, _ = LSTM.lstm(1)  # 预测时只输入[1,time_step,input_size]的测试数据
...
class SAVE:
    def saves(i, predicts):
        t1 = Timer("DB_Read()", "from __main__ import DB_Read")
...
def rnn(i, datas):
    LSTM_Initials.initials(datas)
    TRAIN_LSTM.train_lstm(i)
    a = PREDICTION.prediction(i)
    SAVE.saves(i,a)
...
if __name__ == '__main__':
    ...
    try:
        datas = DB_Read()
        # t1 = Timer("DB_Read()","from __main__ import DB_Read")
        # print('读取数据花费时间：', t1.timeit(2))
        # print(type(t1.timeit(0)))
        # print(datas)
